...
DELAY=round(Delay*SAMPLERATE); % basic delay in # samples                     #EXP: DELAY: 10, WIDTH: 10
WIDTH=round(Width*SAMPLERATE); % modulation width in # samples
if WIDTH>DELAY 
  error('delay greater than basic delay !!!');
  return;
end
MODFREQ=Modfreq/SAMPLERATE; % modulation frequency in # samples               #EXP: MODFREQ = 0.1
LEN=length(x);        % # of samples in WAV-file                              #EXP: LEN
L=2+DELAY+WIDTH*2;    % length of the entire delay                            #EXP: 2 + 10 + 20 = 32
Delayline=zeros(L,1); % memory allocation for delay
y=zeros(size(x));     % memory allocation for output vector
for n=1:LEN
   M=MODFREQ;
   MOD=sin(M*2*pi*n);
   TAP=1+DELAY+WIDTH*MOD;                                                    %#EXP: TAP = 1 + 10 + 10 * sin()
   i=floor(TAP);
   frac=TAP-i;
